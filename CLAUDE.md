...
arai: a typed, ephemeral, event-sourced blackboard for coordinating AI agents.
...
Goal: a Claude Code subagent can post / claim / read against a running `arai mcp` server.
...
4. `packages/cli` — wire `arai mcp` to start the server with a JSONL log under
   `.arai/session-<timestamp>.ndjson`.
...
- `@arai/core` is PURE: no I/O, no `Date.now()`, no randomness, no `node:` imports,
...
- Core's only runtime dependency is `zod`. **Adding ANY dependency anywhere requires
  asking Michael first.** Pre-approved exception: `ulid` in `@arai/transport-mcp`.
...
packages/cli            arai bin: init | mcp | studio | replay
